...
setup_ssl() {
    ...
    # Usar servidor DNS público para evitar cache
    DNS_IP=$(dig @8.8.8.8 +short "$DOMAIN" 2>/dev/null | tail -n1 || dig @1.1.1.1 +short "$DOMAIN" 2>/dev/null | tail -n1 || dig +short "$DOMAIN" 2>/dev/null | tail -n1 || echo "")
    if [ -z "$DNS_IP" ]; then
        ...
        # Avisar se IP parece errado (começa com 216.172)
        if [[ "$DNS_IP" == "216.172"* ]]; then
            warning "DNS pode estar apontando para IP antigo ($DNS_IP)"
            info "Verifique se o DNS está configurado corretamente no painel"
            info "IP esperado: 186.227.125.170"
            echo ""
            read -p "DNS está correto no painel? (s/n): " -n 1 -r
            echo
            if [[ ! $REPLY =~ ^[Ss]$ ]]; then
                error "Corrija o DNS primeiro e tente novamente"
                exit 1
            fi
        fi
    fi
